...
struct HttpBodyProvider {
  private let boundary = URLRequestProvider.boundary
  
  func setPostBody(product: ProductRegisterRequest, apiRequestType: APIRequestType, completionHandler: @escaping (Result<URLRequest, OpenMarketError>) -> ()) {
    do {
      var urlRequest = try URLRequestProvider.makeURLRequest(httpMethod: .post, apiRequestType: apiRequestType)
  ...
  func setUpdateBody(product: ProductUpdateRequest, apiRequestType: APIRequestType, completionHandler: @escaping (Result<URLRequest, OpenMarketError>) -> ()) {
    do {
      var urlRequest = try URLRequestProvider.makeURLRequest(httpMethod: .patch, apiRequestType: apiRequestType)
      urlRequest.httpBody = makeHttpBody(product: product)
      completionHandler(.success(urlRequest))
    } catch {
      completionHandler(.failure(.invalidRequest))
  ...
  func setDeleteBody(product: ProductDeleteRequest, apiRequestType: APIRequestType, completionHandler: @escaping (Result<URLRequest, OpenMarketError>) -> ()) {
    var urlRequest: URLRequest?
    var encodedData: Data = Data()
    
    do {
      urlRequest = try URLRequestProvider.makeURLRequest(httpMethod: .delete, apiRequestType: apiRequestType)
    } catch {
      completionHandler(.failure(.invalidRequest))
    }
